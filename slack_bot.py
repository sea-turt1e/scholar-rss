...
def create_summarized_message(result: arxiv.Result, language=DEFAULT_LANGUAGE, show_url=True) -> str:
...
def post_summary() -> None:
    ...
    for i, result in enumerate(tqdm(result_dict.values(), desc="request to semantic scholar")):
        ...
        if arxiv_url:
            ...
            if arxiv_id not in already_summarized:
                if (sem.get("year", 0) >= int(today_date[:4])) and (
                    (num_cited_by >= NUM_CITED_BY) or (citation_velocity > 0)
                ):
                    summary_candidates[arxiv_id] = {
                        "result": result,
    ...
    if len(summary_candidates) > NUM_SUMMARIZE:
        summary_candidates = {
            k: v
            for k, v in sorted(summary_candidates.items(), key=lambda x: x[1]["num_cited_by"], reverse=True)[
                :NUM_SUMMARIZE
            ]
        }
...
def reply_message(body: dict, say: Say) -> None:
    ...
    if user_msg_text[1:5] == "http":  # arxivのURLが入力された時
        ...
        if not is_query_summarized:
            say(
                "クエリからうまく論文が見つけられませんでした。クエリを変えてもう一度入力してください。",
...
if __name__ == "__main__":
    ...
    # p1.start()
    # p2.start()
    print(get_arxiv_ids_from_query(query="Deep Learning"))
